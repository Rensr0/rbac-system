...
| # | 任务 | 说明 | 状态 |
|---|------|------|------|
| 71 | 修改 API `/api/log/index.php` 权限逻辑 | 普通用户可查看自己的全部操作记录（不限于登录/退出），超级管理员查看全部 | ⬜ 待完成 |
| 72 | 移动端个人中心移除登录记录板块 | 删除 `app-mine.js` 中的 `mine-login-logs` 相关代码 | ⬜ 待完成 |
| 73 | PC 端个人中心移除登录记录板块 | 删除 `pc-pages.js` 中 `loadPCMine` 的 `pc-mine-logs` 相关代码 | ⬜ 待完成 |
| 74 | 验证移动端操作记录页面正常工作 | 普通用户只能看到自己的操作记录 | ⬜ 待完成 |
| 75 | 验证 PC 端操作记录页面正常工作 | 超级管理员看到全部，普通用户看到自己的 | ⬜ 待完成 |
| 76 | 提交并推送到 GitHub | 包含所有改动 | ⬜ 待完成 |
