...
> 最近更新：2026-03-25 07:35 GMT+8（二轮开发后）
...
- **无导入导出**：不能从 CSV/Excel 导入用户，目前仅支持导出为 CSV（已有导出按钮）
...
| 路由权限细分 | 至少支持"查看/编辑/删除"三级，而非简单的"有/无" | ❌ 待开发 |
| 数据导入导出 | Excel/CSV 格式，用户和角色都支持（已有CSV导出，需补导入） | 🔄 部分完成 |
...
| 用户头像上传 | 支持上传自定义头像 | ❌ 待开发 |
| 长列表虚拟滚动 | 用户/角色多时性能优化 | ❌ 待开发 |
...
- **app.js 仍然较大**：建议按模块拆分为 `app-users.js`、`app-roles.js`、`app-routers.js`、`app-mine.js`
- **PC/移动端代码重复度高**：`editRouterApp`（移动）和 `pcEditRouter`（PC）逻辑几乎一样，建议抽出共用函数
...
当前 v3.0 版本经过两轮开发，核心功能已完整可用，体验层面也有显著提升：

**本轮完成的工作（15 项）：**
- 登录页全面优化（Tab动画/验证码反馈/密码显隐/注册引导/密码强度）
- 移动端适配修复（Tab溢出、首页重复入口、弹窗层级）
- 新增功能（审计日志、404页面、批量操作、数据可视化升级）
- 体验打磨（网络异常处理、编辑确认、搜索过滤、弹窗溢出）

**仍需关注的方向：**
1. **权限粒度**——只有"有/无"，没有"读/写/删"分级（P0）
2. **数据管理**——导入功能待补全（P0）
3. **头像系统**——上传自定义头像（P1）
4. **性能优化**——长列表虚拟滚动（P1）
...
*文档更新时间：2026-03-25 07:35 GMT+8*
*更新说明：根据 git 提交记录对照 FUTURE_PLAN.md，标记已完成项，精简仍存在问题清单*
