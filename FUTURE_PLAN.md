...
| # | 改进项 | 提交 |
|---|--------|------|
| 31 | PC端图标选择器改用 div 容器，修复 select 内 onclick 不触发 | 待提交 |
| 32 | Material Icons 添加 pointer-events: none，修复主题按钮点击穿透 | 待提交 |
| 33 | 移除错位的 privacy_policy 内置图标 | 待提交 |
...
当前 v3.1 版本经过三轮开发，核心功能完整、权限粒度大幅提升：

**本轮完成的工作（第三轮，5 项）：**
- 路由权限细分系统（查看/编辑/删除三级，位掩码存储）
- 角色编辑器全面升级（PC+移动端三级权限复选框，级联勾选）
- 角色列表显示权限级别标签
- 服务器端批量用户导入接口（替代前端逐条创建）
- 数据库自动迁移机制（兼容已有部署）

**累计完成（30 项）：** P0 全部完成，P1 完成 8/8

**仍需关注的方向：**
1. **长列表虚拟滚动** — 性能优化（P1）
2. **P2 迭代** — 路由层级、角色继承、多语言等

---

*文档更新时间：2026-03-25 11:35 GMT+8*
*更新说明：第五轮开发 — 修复PC端图标选择器不可点击、主题按钮icon点击穿透、移除错位的privacy_policy图标*
