...
| # | 改进项 | 提交 | 原优先级 |
|---|--------|------|----------|
| 35 | 移动端用户列表无限滚动（IntersectionObserver + 分页加载） | `待提交` | P1 |
| 36 | 移动端日志列表无限滚动（IntersectionObserver + 分页加载） | `待提交` | P1 |
...
当前 v3.2 版本经过六轮开发，核心功能完整、权限粒度大幅提升：

**本轮完成的工作（第六轮，2 项）：**
- 移动端用户列表无限滚动（IntersectionObserver，滚动到接近底部自动加载下一页）
- 移动端日志列表无限滚动（同上）

**累计完成（36 项）：** P0 全部完成，P1 全部完成

**仍需关注的方向：**
1. **P2 迭代** — 路由层级、角色继承、多语言等

---

*文档更新时间：2026-03-25 12:52 GMT+8*
*更新说明：第六轮开发 — 移动端用户/日志列表无限滚动，替代一次性加载20条无翻页* 
